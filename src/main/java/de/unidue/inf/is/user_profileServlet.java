	...
	request.setAttribute("reasons", "");
		...
		while (resultSet.next()){	
    	 ...
    	 try {	//SELECT text,created,creator,id FROM babble WHERE creator = ? ORDER BY id DESC
 			myConnection = myDB.getConnection("babble");	//
 			PreparedStatement myPrepStatement = myConnection.prepareStatement("SELECT b.text,b.created,b.creator,b.id, count(lb.babble) AS likes FROM babble b, LikesBabble lb WHERE b.id = lb.babble AND lb.type = 'likes' AND b.creator = ? GROUP BY  b.text,b.created,b.creator,b.id ORDER BY b.id DESC");
 			myPrepStatement.setString(1, userID);
 			ResultSet resultSet = myPrepStatement.executeQuery();
